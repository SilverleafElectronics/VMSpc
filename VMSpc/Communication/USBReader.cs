...
namespace VMSpc.Communication
{
    class USBReader : DataReader
    {
        ...
        public USBReader(ushort comPort)
            : base()
        {
        ...
        private void HandleCommPortData(object sender, SerialDataReceivedEventArgs e)
        {
            if (Application.Current != null)
            {
                Application.Current.Dispatcher.BeginInvoke(new SplitDataIntoProcess(() =>
                {
                    ...
                    try
                    {
                        string[] buffer = SplitData(portReader.ReadExisting());
                        if (buffer != null)
                        {
        ...
        /// <summary>
        /// Splits the data by newline characters
        /// </summary>
        /// <param name="buffer"></param>
        /// <returns></returns>
        protected virtual string[] SplitData(string buffer)
        {
            return buffer.Split('\n');
        }
